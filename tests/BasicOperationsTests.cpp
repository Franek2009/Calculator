...
{
    REQUIRE(BasicOperations::add(2, 3) == 5);
}

TEST_CASE("Subtraction works correctly")
{
    REQUIRE(BasicOperations::subtract(5, 3) == 2);
}

TEST_CASE("Multiplication works correctly")
{
    REQUIRE(BasicOperations::multiply(2, 3) == 6);
}

TEST_CASE("Division works correctly")
{
    REQUIRE(BasicOperations::divide(6, 3) == 2);
}

TEST_CASE("Power works correctly")
{
    REQUIRE(BasicOperations::power(2, 3) == 8);
}

TEST_CASE("Square root works correctly")
{
    REQUIRE(BasicOperations::squareRoot(4) == 2);
}

TEST_CASE("Division by zero throws an exception")
{
    REQUIRE_THROWS_AS(BasicOperations::divide(6, 0), std::invalid_argument);
}

TEST_CASE("Square root of a negative number throws an exception")
{
    REQUIRE_THROWS_AS(BasicOperations::squareRoot(-1), std::invalid_argument);
}
